...
- No instalado: no aparece en `settings.SYSTEM_MODULES`, no publica rutas, y un deep link viejo puede dar `404`.
...
py -3 manage.py test system_modules.tests turnos.tests.test_module_architecture turnos.tests.test_module_guards --settings=config.settings_test
```
...
2. Registrar el descriptor en `settings.SYSTEM_MODULES`.
