...
- Retirar entrypoints publicos top-level de views, forms, services, selectors, signals, api views, serializers, templates, static y realtime.
...
- Templates y static se cargan desde `*/interfaces/templates` y `*/interfaces/static`.
- Test de arquitectura exige layout literal, prohibe rutas declaradas fuera de `interfaces/`, entrypoints publicos top-level e imports legacy.
- La frontera publica queda en `interfaces/`; no hay wrappers top-level aceptados como contrato final.
