...
def set_module_enabled(slug, enabled):
    registry = get_module_registry()
    if not registry.has(slug):
        raise ValueError(f"Modulo no instalado: {slug}")

    sync_installed_modules()
    state = ModuleState.objects.get(slug=slug)
    if enabled and not state.is_installed:
        raise ValueError(f"Modulo no instalado: {slug}")
