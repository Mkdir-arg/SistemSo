...
class ConfiguracionTurnosForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'anticipacion_minima_hs': forms.NumberInput(attrs={'class': 'form-input w-full rounded-lg border-gray-300', 'min': 0}),
            'anticipacion_maxima_dias': forms.NumberInput(attrs={'class': 'form-input w-full rounded-lg border-gray-300', 'min': 1}),
            'cancelacion_hasta_hs': forms.NumberInput(attrs={'class': 'form-input w-full rounded-lg border-gray-300', 'min': 0}),
        }
...
class DisponibilidadConfiguracionForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'hora_inicio': forms.TimeInput(attrs={'type': 'time', 'class': 'form-input w-full rounded-lg border-gray-300'}),
            'hora_fin': forms.TimeInput(attrs={'type': 'time', 'class': 'form-input w-full rounded-lg border-gray-300'}),
            'duracion_turno_min': forms.NumberInput(attrs={'class': 'form-input w-full rounded-lg border-gray-300', 'min': 5, 'step': 5}),
            'cupo_maximo': forms.NumberInput(attrs={'class': 'form-input w-full rounded-lg border-gray-300', 'min': 1}),
        }
    ...
    def clean(self):
        ...
        if hora_inicio and hora_fin:
            ...
            if duracion:
                ...
                slots = minutos_totales // duracion
                resto = minutos_totales % duracion

                cleaned['_slots_preview'] = slots
                cleaned['_resto_min'] = resto
